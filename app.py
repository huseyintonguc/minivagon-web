...
def create_pdf(s, urun_dict):
    ...
    pdf.text(60, 5, f"Siparis No: #{s.get('Siparis No')}")
    pdf.text(60, 9, f"Tarih: {s.get('Tarih')}")
            ...
            if os.path.exists(full_path):
                try:
                    with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as tmp:
                        ...
                        img.save(tmp.name)
                        pdf.image(tmp.name, x=x_pos, y=14, w=25) 
                except Exception as e:
                    print("Resim hatasi:", e)

    if s.get('Ürün 2'): 
        resim_koy(s.get('Ürün 1'), 20)
...
def create_pazaryeri_pdf(s, urun_dict):
    ...
    pdf.set_text_color(255, 255, 255)
    set_ft('B', 10) 
    pdf.text(3, 8, "AHSAP HOBI DUNYASI - PAZARYERI KART")
